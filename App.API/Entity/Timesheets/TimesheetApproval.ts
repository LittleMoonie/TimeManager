...
export class TimesheetApproval extends BaseEntity {
  ...
  @ManyToOne(() => Timesheet, { onDelete: 'CASCADE' }) @JoinColumn({ name: 'timesheetId' }) timesheet!: Timesheet;

  @Column('uuid') approverId!: string;
  @ManyToOne(() => User, { onDelete: 'RESTRICT' }) @JoinColumn({ name: 'approverId' }) approver!: User;
