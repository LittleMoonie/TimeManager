...
export class UserStatus extends BaseEntity {
  @Column({ type: 'varchar', length: 50, unique: true, nullable: false }) code!: string; // e.g. INVITED, ACTIVE, SUSPENDED, TERMINATED
  
  @Column({ type: 'varchar', length: 50, unique: true, nullable: false }) name!: string; // Display label
