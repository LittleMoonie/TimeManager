...
import { Permission } from '../Entity/Users/Permission';
import { RolePermission } from '../Entity/Users/RolePermission';
...
export async function seedRolesAndPermissions(ds: DataSource, company: Company) {
  ...
  const permissions = [
    ...
    'permissions:read',
    'timesheets:submit','timesheets:approve',
    'sessions:revoke'
  ];
  ...
  await roleRepo.upsert(
    roleDefs.map(r => ({ companyId: company.id, name: r.name, description: r.description })),
    { conflictPaths: ['companyId', 'name'] }
  );
