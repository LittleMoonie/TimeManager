...
export async function seedUsers(ds: DataSource, company: Company, roles: RolesMap, statuses: StatusMap) {
  ...
  const makeUser = async (email: string, firstName: string, lastName: string, roleName: string) => {
    ...
    let user = await userRepo.findOne({ where: { companyId: company.id, email } });
    if (!user) {
