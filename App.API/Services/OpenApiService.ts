...
export class OpenApiService {
  ...
  static async needsRegeneration(): Promise<boolean> {
    try {
          ...
          for (const file of files) {
            ...
            if (file.isDirectory()) {
              ...
              if (fileStats.mtime > swaggerTime) {
                console.log(
                  `📝 File ${filePath} is newer than swagger.json, regeneration needed`,
                );
                return true;
              }
            }
      ...
      return (
        (await checkDirectory(controllersPath)) ||
        (await checkDirectory(dtosPath))
      );
    } catch (error) {
      console.log(
        "📝 Error checking regeneration need, assuming needed:",
        error,
      );
      return true;
    }
