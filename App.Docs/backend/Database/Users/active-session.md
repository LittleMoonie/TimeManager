...
The `ActiveSession` entity is crucial for managing user authentication sessions, particularly refresh tokens. It tracks active login instances, allowing for secure session management, revocation, and monitoring of user activity. This entity plays a key role in the JWT-based authentication flow.

This entity is defined in `App.API/Entities/Users/ActiveSessions.ts`.
...
In addition to the fields inherited from `BaseEntity`, the `ActiveSession` entity includes:

| Column Name         | Type                 | Description                                                               |
| :------------------ | :------------------- | :------------------------------------------------------------------------ |
| `companyId`         | `UUID`               | Foreign Key to `Company.id`, linking the session to its organization.     |
| `userId`            | `UUID`               | Foreign Key to `User.id`, identifying the user who owns the session.      |
| `tokenHash`         | `TEXT`               | SHA-256 hash of the refresh token for secure storage.                     |
| `previousTokenHash` | `TEXT` (nullable)    | Hash of the previous refresh token, used for refresh token rotation.      |
| `expiresAt`         | `TIMESTAMPTZ` (nullable) | Expiration date and time of the refresh token.                            |
| `revokedAt`         | `TIMESTAMPTZ` (nullable) | Timestamp when the session was revoked (e.g., on logout).                 |
| `lastSeenAt`        | `TIMESTAMPTZ` (nullable) | Timestamp of the last activity for this session.                          |
| `ip`                | `INET` (nullable)    | IP address from which the session originated.                             |
| `userAgent`         | `TEXT` (nullable)    | User agent string of the client.                                          |
| `deviceId`          | `TEXT` (nullable)    | Unique identifier for the device associated with the session.             |
...
To optimize query performance and ensure data integrity, the `ActiveSession` entity defines the following indexes:

*   `@Index(["companyId", "userId"])`: Composite index for efficient lookups by user within a company.
*   `@Index(["tokenHash"], { unique: true })`: Unique index on the token hash to ensure each refresh token is unique.
*   `@Index(["expiresAt"])`: Index on expiration date for efficient cleanup of expired sessions.

## Constraints

*   **Unique Token Hash**: Ensures that each refresh token hash is unique.
