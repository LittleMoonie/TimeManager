...
Roles are attached to a user _per company_ and define a set of permissions applicable only within that company.
...
Permission keys follow the format `resource.action.scope` and are evaluated _per company_. The `scope` can be `self`, `team`, or `org` (company-wide).
...
- **`manager`**
  - _Inherits all `employee` permissions_
  - `timesheet.view.team`
...
- **`hr`**
  - _Inherits all `manager` permissions_
  - `timesheet.view.org`
...
- **`company_admin`**
  - _Inherits all `hr` permissions_
  - _Inherits all `payroll` permissions_
  - _Inherits all `auditor` permissions_
  - `rbac.manage.company`
