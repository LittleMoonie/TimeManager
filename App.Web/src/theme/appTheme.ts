import { alpha, createTheme as createTheme, responsiveFontSizes } from '@mui/material/styles';
...
export type PrimaryColor = 'purple' | 'blue' | 'teal';

export const primarySwatches: Record<PrimaryColor, { light: string; main: string; dark: string }> = {
  purple: {
    light: '#8E6CD9',
    main: '#5E35B1',
    dark: '#4527A0',
...
const buildShadows = (mode: PaletteMode) => {
  const base = createTheme({ palette: { mode } }).shadows;
...
const getPalette = (mode: PaletteMode, primaryColor: PrimaryColor) => {
  const primary = primarySwatches[primaryColor];
  if (mode === 'light') {
    return {
      mode,
      primary,
...
export const createAppTheme = (
  ...
  const palette = getPalette(mode, primaryColor);
